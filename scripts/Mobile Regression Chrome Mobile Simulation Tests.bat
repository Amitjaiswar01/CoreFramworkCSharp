...
set traits=-trait "Category=Chrome Mobile Simulation"
...
set resultsFileName=%projectDirectory%Results\%currentDate% %currentTime% %releaseName% Mobile Regression Chrome Mobile Simulation Results

if "%environment%"=="" (
set /p "environment=Select an instance to test on A-I from http://10.1.16.80/ "
)

@echo Environment %environment% Selected

copy /y %path%\Environments\%environment%.txt "%path%\Test Environment.txt"
